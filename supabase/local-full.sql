...
-- Local-only full schema (no auth.users dependency)
-- Uses text user ids and a default demo user: local-demo-user
...
create trigger trg_risk_rules_updated_at before update on public.risk_rules for each row execute function public.set_updated_at();
...
create trigger trg_risk_scores_updated_at before update on public.risk_scores for each row execute function public.set_updated_at();
drop trigger if exists trg_fraud_cases_updated_at on public.fraud_cases;
create trigger trg_fraud_cases_updated_at before update on public.fraud_cases for each row execute function public.set_updated_at();
...
create trigger trg_chargebacks_updated_at before update on public.chargebacks for each row execute function public.set_updated_at();
drop trigger if exists trg_identity_verifications_updated_at on public.identity_verifications;
create trigger trg_identity_verifications_updated_at before update on public.identity_verifications for each row execute function public.set_updated_at();
drop trigger if exists trg_entity_lists_updated_at on public.entity_lists;
create trigger trg_entity_lists_updated_at before update on public.entity_lists for each row execute function public.set_updated_at();
drop trigger if exists trg_model_registry_updated_at on public.model_registry;
create trigger trg_model_registry_updated_at before update on public.model_registry for each row execute function public.set_updated_at();
drop trigger if exists trg_api_integrations_updated_at on public.api_integrations;
create trigger trg_api_integrations_updated_at before update on public.api_integrations for each row execute function public.set_updated_at();
...
-- Local RLS helper: defaults to local-demo-user unless overridden per-connection
create or replace function public.app_user_id()
returns text
language sql
stable
as $$
  select coalesce(current_setting('app.current_user_id', true), 'local-demo-user')
$$;
...
create policy payment_methods_owner_all on public.payment_methods for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists risk_rules_owner_all on public.risk_rules;
create policy risk_rules_owner_all on public.risk_rules for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());
...
create policy risk_scores_owner_all on public.risk_scores for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists fraud_cases_owner_all on public.fraud_cases;
create policy fraud_cases_owner_all on public.fraud_cases for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());
...
create policy chargebacks_owner_all on public.chargebacks for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists behavioral_biometrics_owner_all on public.behavioral_biometrics;
create policy behavioral_biometrics_owner_all on public.behavioral_biometrics for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists account_security_events_owner_all on public.account_security_events;
create policy account_security_events_owner_all on public.account_security_events for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists identity_verifications_owner_all on public.identity_verifications;
create policy identity_verifications_owner_all on public.identity_verifications for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists entity_lists_owner_all on public.entity_lists;
create policy entity_lists_owner_all on public.entity_lists for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists model_registry_owner_all on public.model_registry;
create policy model_registry_owner_all on public.model_registry for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists graph_edges_owner_all on public.graph_edges;
create policy graph_edges_owner_all on public.graph_edges for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists api_integrations_owner_all on public.api_integrations;
create policy api_integrations_owner_all on public.api_integrations for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists compliance_reports_owner_all on public.compliance_reports;
create policy compliance_reports_owner_all on public.compliance_reports for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists channel_events_owner_all on public.channel_events;
create policy channel_events_owner_all on public.channel_events for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

drop policy if exists customer_risk_profiles_owner_all on public.customer_risk_profiles;
create policy customer_risk_profiles_owner_all on public.customer_risk_profiles for all
using (user_id = public.app_user_id())
with check (user_id = public.app_user_id());

create or replace function public.seed_core_feature_data_for_user(target_user text)
returns void
language plpgsql
as $$
declare
...
begin
  ...
  if not exists (select 1 from public.graph_edges where user_id = target_user limit 1) then
    ...
    select t.user_id, t.id, 'user', t.user_id, 'device', t.device_id, 'shared_device', 1.25
...
create or replace function public.seed_demo_data_for_user(target_user text)
returns void
language plpgsql
as $$
declare
...
begin
  ...
  values
    (target_user, 'MacBook Pro - Office', 'fp_mac_office_001', 'desktop', 'macOS 14', 'Chrome', '35.189.12.22', 'US', now() - interval '10 minutes', 'low'),
    (target_user, 'iPhone 15', 'fp_iphone_015', 'mobile', 'iOS 18', 'Safari', '108.72.41.10', 'US', now() - interval '20 minutes', 'low'),
    (target_user, 'Android Tablet', 'fp_tablet_002', 'tablet', 'Android 15', 'Chrome', '192.188.1.34', 'US', now() - interval '2 hours', 'medium'),
    (target_user, 'Windows Remote Node', 'fp_windows_node_77', 'server', 'Windows 2022', 'Edge', '156.146.53.8', 'NG', now() - interval '35 minutes', 'high'),
    (target_user, 'Linux API Runner', 'fp_linux_api_19', 'server', 'Ubuntu 24.04', 'Headless', '44.211.80.11', 'US', now() - interval '45 minutes', 'medium'),
    (target_user, 'Unknown Browser Session', 'fp_unknown_999', 'other', 'Unknown', 'Unknown', '102.10.18.73', 'PH', now() - interval '15 minutes', 'high')
  on conflict (user_id, device_fingerprint) do nothing;

  select id into d1 from public.devices where user_id = target_user and device_fingerprint = 'fp_mac_office_001';
  select id into d2 from public.devices where user_id = target_user and device_fingerprint = 'fp_iphone_015';
  select id into d3 from public.devices where user_id = target_user and device_fingerprint = 'fp_tablet_002';
  select id into d4 from public.devices where user_id = target_user and device_fingerprint = 'fp_windows_node_77';
  select id into d5 from public.devices where user_id = target_user and device_fingerprint = 'fp_linux_api_19';
  select id into d6 from public.devices where user_id = target_user and device_fingerprint = 'fp_unknown_999';

  insert into public.payment_methods (user_id, provider, method_type, last4, bin, country, fingerprint, status, is_default)
  values
    (target_user, 'Visa', 'card', '4242', '424242', 'US', 'pm_fp_4242', 'active', true),
    (target_user, 'Mastercard', 'card', '5100', '510510', 'US', 'pm_fp_5100', 'active', false),
    (target_user, 'ACH - Chase', 'bank', '9921', null, 'US', 'pm_fp_ach_9921', 'active', false),
    (target_user, 'Apple Pay', 'wallet', '3005', null, 'US', 'pm_fp_wallet_3005', 'active', false),
    (target_user, 'Virtual BIN', 'virtual_card', '8821', '482901', 'NG', 'pm_fp_virtual_8821', 'blocked', false)
  on conflict (user_id, fingerprint) do nothing;

  select id into pm1 from public.payment_methods where user_id = target_user and fingerprint = 'pm_fp_4242';
  select id into pm2 from public.payment_methods where user_id = target_user and fingerprint = 'pm_fp_5100';
  select id into pm3 from public.payment_methods where user_id = target_user and fingerprint = 'pm_fp_ach_9921';
  select id into pm4 from public.payment_methods where user_id = target_user and fingerprint = 'pm_fp_wallet_3005';
  select id into pm5 from public.payment_methods where user_id = target_user and fingerprint = 'pm_fp_virtual_8821';
  ...
  values
    (target_user, 'AtlasPay Checkout', 'digital_goods', 1299, 'USD', 'review', 62, 'card', pm1, '35.189.12.22', 'US', 'new-device-1289', d1, 3, 9, 32.50, now() - interval '5 minutes'),
    (target_user, 'Nova Wallet', 'wallet_topup', 244, 'USD', 'approved', 24, 'wallet', pm4, '108.72.41.10', 'US', 'iphone-14-pro', d2, 1, 2, 7.20, now() - interval '12 minutes'),
    (target_user, 'CrossBorder Transfer', 'p2p', 3400, 'USD', 'blocked', 84, 'virtual_card', pm5, '156.146.53.8', 'NG', 'new-device-xy77', d4, 7, 18, 78.40, now() - interval '18 minutes'),
    (target_user, 'QuickMart', 'retail', 58, 'USD', 'approved', 11, 'card', pm2, '44.211.80.11', 'US', 'macbook-office', d1, 1, 3, 3.10, now() - interval '26 minutes'),
    (target_user, 'CloudScale', 'saas', 899, 'USD', 'review', 49, 'bank', pm3, '44.211.80.11', 'US', 'linux-api', d5, 2, 5, 25.50, now() - interval '33 minutes'),
    (target_user, 'NightOwl Bets', 'gaming', 1200, 'USD', 'blocked', 76, 'card', pm2, '102.10.18.73', 'PH', 'unknown-browser', d6, 6, 12, 66.70, now() - interval '47 minutes'),
    (target_user, 'MetroRide', 'transport', 24, 'USD', 'approved', 8, 'wallet', pm4, '108.72.41.10', 'US', 'iphone-14-pro', d2, 1, 1, 1.50, now() - interval '1 hour'),
    (target_user, 'Pixel Market', 'digital_goods', 430, 'USD', 'review', 55, 'card', pm1, '35.189.12.22', 'US', 'new-device-1289', d1, 4, 10, 34.20, now() - interval '2 hours'),
    (target_user, 'HyperComms', 'telecom', 74, 'USD', 'approved', 16, 'bank', pm3, '44.211.80.11', 'US', 'linux-api', d5, 1, 2, 4.80, now() - interval '3 hours'),
    (target_user, 'Global Flights', 'travel', 2180, 'USD', 'review', 68, 'card', pm2, '156.146.53.8', 'NG', 'new-device-xy77', d4, 5, 11, 52.10, now() - interval '4 hours'),
    (target_user, 'FoodClub', 'food', 42, 'USD', 'approved', 13, 'wallet', pm4, '108.72.41.10', 'US', 'iphone-14-pro', d2, 1, 4, 2.20, now() - interval '5 hours'),
    (target_user, 'CryptoSwap', 'crypto', 970, 'USD', 'blocked', 81, 'crypto', pm5, '102.10.18.73', 'PH', 'unknown-browser', d6, 8, 21, 74.90, now() - interval '6 hours');

  select id into tx1 from public.transactions where user_id = target_user and merchant_name = 'AtlasPay Checkout' order by created_at desc limit 1;
  select id into tx2 from public.transactions where user_id = target_user and merchant_name = 'Nova Wallet' order by created_at desc limit 1;
  select id into tx3 from public.transactions where user_id = target_user and merchant_name = 'CrossBorder Transfer' order by created_at desc limit 1;
  select id into tx6 from public.transactions where user_id = target_user and merchant_name = 'NightOwl Bets' order by created_at desc limit 1;
  select id into tx8 from public.transactions where user_id = target_user and merchant_name = 'Pixel Market' order by created_at desc limit 1;
  select id into tx10 from public.transactions where user_id = target_user and merchant_name = 'Global Flights' order by created_at desc limit 1;
  select id into tx12 from public.transactions where user_id = target_user and merchant_name = 'CryptoSwap' order by created_at desc limit 1;

  insert into public.risk_scores (user_id, transaction_id, model_version, score, decision, reason_codes, explanation)
  select target_user, t.id, 'fraud-v3.2.1', t.risk_score, t.status, array['model_signal'], '{"source":"local_seed"}'::jsonb
  from public.transactions t
  where t.user_id = target_user;
  ...
  select id into c1 from public.fraud_cases where user_id = target_user and title = 'Velocity spike detected' limit 1;
  select id into c2 from public.fraud_cases where user_id = target_user and title = 'High-risk geo mismatch' limit 1;
  select id into c3 from public.fraud_cases where user_id = target_user and title = 'Gaming abuse pattern' limit 1;
  select id into c4 from public.fraud_cases where user_id = target_user and title = 'Digital goods anomaly' limit 1;
  select id into c5 from public.fraud_cases where user_id = target_user and title = 'Crypto payout risk' limit 1;
...
insert into public.profiles (id, first_name, last_name, email, role, timezone)
values ('local-demo-user', 'Local', 'Analyst', 'local@aegis.dev', 'admin', 'UTC')
on conflict (id) do nothing;

select public.seed_demo_data_for_user('local-demo-user');
